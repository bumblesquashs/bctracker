...
def load(system, agency, force_download=False, update_db=False):
    ...
    try:
        exceptions = read_csv(system, agency, 'calendar_dates', lambda r: ServiceException.from_csv(r, system))
        service_exceptions = {}
        for exception in exceptions:
            service_exceptions.setdefault(exception.service_id, []).append(exception)
        
        try:
            services = read_csv(system, agency, 'calendar', lambda r: Service.from_csv(r, system, agency, service_exceptions))
        except:
...
def download(system, agency):
    ...
    try:
        ...
        with requests.get(agency.get_gtfs_url(system), stream=True) as r:
...
def read_csv(system, agency, name, initializer):
