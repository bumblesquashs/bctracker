    ...
    <div class="stop">
        % if get('show_name', True):
            % if get('include_link', True):
                <a class="stop-name {{ 'timing-point' if get('timepoint', False) else '' }}" href="{{ get_url(stop.system, 'stops', stop) }}">{{ stop }}</a>
            % else:
                <span class="stop-name {{ 'timing-point' if get('timepoint', False) else '' }}">{{ stop }}</span>
            % end
        % end
        % if get('show_number', True) and stop.system.agency.show_stop_number:
