...
def start(args):
    ...
    for system in helpers.system.find_all():
        ...
        if running:
            gtfs.load(system, agency, args.reload, args.updatedb)
            if not gtfs.validate(system, agency):
                gtfs.load(system, agency, True)
            gtfs.update_cache_in_background(system)
            realtime.update(system, agency)
            if not realtime.validate(system, agency):
...
def get_url(system=None, agency=None, path='', **kwargs):
    '''Returns a URL formatted based on the given system and path'''
    system_id = getattr(system, 'id', system)
    agency_id = getattr(agency, 'id', agency)
    if system_id:
        url = config.system_domain.format(system_id) + path
    elif agency_id:
        url = config.agency_domain.format(agency_id) + path
    else:
        url = config.global_domain + path
...
def endpoint(base_path, method='GET', append_slash=True, require_admin=False, system_key='system_id', agency_key='agency_id'):
...
def bus_overview_page(system, agency, bus_number):
    ...
    records = helpers.record.find_all(agency=agency, bus=bus, limit=20)
...
def route_schedule_date_page(system, agency, route_number, date_string):
    if system is None:
        return error_page('system_required',
            title='System Required',
            path=f'/routes/{route_number}/schedule/{date_string}',
            system=system,
            agency=agency
        )
    route = system.get_route(number=route_number)
...
def stop_schedule_date_page(system, agency, stop_number, date_string):
    if system is None:
        return error_page('system_required',
            title='System Required',
            path=f'/stops/{stop_number}/schedule/{date_string}',
            system=system,
            agency=agency
        )
    stop = system.get_stop(number=stop_number)
...
def api_search(system, agency):
    ...
    max = min + count
    return {
        'results': [m.get_json(system, agency, get_url) for m in matches[min:max]],
        'total': len(matches)
    }
