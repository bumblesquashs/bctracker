...
class Server(Bottle):
    ...
    def __init__(self, database: Database, settings: Settings):
        ...
        self.add('/bus/<bus_agency_id>/<bus_number>', callback=self.bus_overview)
        self.add('/bus/<bus_agency_id>/<bus_number>/map', callback=self.bus_map)
        self.add('/bus/<bus_agency_id>/<bus_number>/history', callback=self.bus_history)
    ...
    def bus_overview(self, context: Context, bus_number, bus_agency_id=None):
        if bus_agency_id:
            context.agency = repositories.agency.find(bus_agency_id)
        ...
        overview = repositories.overview.find(context, bus)
    ...
    def bus_map(self, context: Context, bus_number, bus_agency_id=None):
        if bus_agency_id:
            context.agency = repositories.agency.find(bus_agency_id)
        if not context.agency:
            return self.error_page(
    ...
    def bus_history(self, context: Context, bus_number, bus_agency_id=None):
        if bus_agency_id:
            context.agency = repositories.agency.find(bus_agency_id)
        if not context.agency:
            return self.error_page(
        ...
        match selection:
            case 'bus':
                ...
                if system:
                    ...
                    redirect(self.get_url(context, 'bus', overview.context.agency, overview.bus))
            case 'route':
                routes = repositories.route.find_all(context)
                if not routes:
                    redirect(self.get_url(context))
