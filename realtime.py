...
def update(system, agency):
    '''Downloads realtime data and stores it in the database'''
    ...
    try:
        ...
        for index, entity in enumerate(data.entity):
            ...
            try:
                ...
                vehicle_name_length = agency.vehicle_name_length
                ...
                bus_number = -(index + 1)
            helpers.position.create(system, agency, bus_number, vehicle)
        last_updated_date = Date.today()
        last_updated_time = Time.now(accurate_seconds=False)
        system.last_updated_date = Date.today(system.timezone)
        system.last_updated_time = Time.now(system.timezone, agency.accurate_seconds)
    ...
    try:
        for position in helpers.position.find_all():
            try:
                ...
                overview = helpers.overview.find(agency, bus.number)
                trip = position.trip
                if trip is None:
                    record_id = None
                else:
                    ...
                    if overview is not None and overview.last_record is not None:
                        ...
                        if last_record.date == date and last_record.block_id == block.id:
                            ...
                            continue
                    record_id = helpers.record.create(agency, bus, date, system, block, time, trip)
                if overview is None:
                    helpers.overview.create(agency, bus, date, system, record_id)
                else:
                    helpers.overview.update(overview, date, system, record_id)
                    if overview.last_seen_system != system:
                        helpers.transfer.create(agency, bus, date, overview.last_seen_system, system)
            except Exception as e:
...
def validate(system, agency):
    '''Checks that the realtime data aligns with the current GTFS'''
    if not agency.realtime_enabled:
        return True
    for position in helpers.position.find_all(system, agency):
