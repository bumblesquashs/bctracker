...
def update(system, agency):
    ...
    try:
        ...
        helpers.position.delete_all(system, agency)
                ...
                if trip is None:
                    ...
                    if overview is not None and overview.last_record is not None:
                        ...
                        if last_record.date == date and last_record.block_id == block.id:
                            ...
                            continue
                    record_id = helpers.record.create(system, agency, bus, date, block, time, trip)
                if overview is None:
                    helpers.overview.create(system, agency, bus, date, record_id)
                else:
...
def validate(system, agency):
    '''Checks that the realtime data aligns with the current GTFS data'''
