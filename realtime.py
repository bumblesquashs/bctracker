...
class Realtime:
    ...
    def update(self, system):
        ...
        try:
            ...
            services.position.delete_all(system)
            for index, entity in enumerate(data.entity):
                ...
                try:
                    ...
                    bus_number = -(index + 1)
                services.position.create(system, bus_number, vehicle)
    ...
    def update_records(self):
        ...
        try:
            for position in services.position.find_all():
                try:
                    ...
                    overview = services.overview.find(bus.number)
                    trip = position.trip
                    if trip is None:
                        record_id = None
                    else:
                        block = trip.block
                        assignment = services.assignment.find(system, block)
                        if not assignment or assignment.bus_number != bus.number:
                            services.assignment.delete_all(system=system, block=block)
                            services.assignment.delete_all(bus=bus)
                            services.assignment.create(system, block, bus, date)
                        if overview is not None and overview.last_record is not None:
                            ...
                            if last_record.date == date and last_record.block_id == block.id:
                                services.record.update(last_record, time)
                                trip_ids = services.record.find_trip_ids(last_record)
                                if trip.id not in trip_ids:
                                    services.record.create_trip(last_record, trip)
                                continue
                        record_id = services.record.create(bus, date, system, block, time, trip)
                    if overview is None:
                        services.overview.create(bus, date, system, record_id)
                    else:
                        services.overview.update(overview, date, system, record_id)
                        if overview.last_seen_system != system:
                            services.transfer.create(bus, date, overview.last_seen_system, system)
                except Exception as e:
    ...
    def validate(self, system):
        ...
        if not system.realtime_enabled:
            return True
        for position in services.position.find_all(system):
