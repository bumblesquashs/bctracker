...
class Trip:
    def __init__(self, tripid, routeid, serviceid, routenum, blockid, headsign, starttime, startstopname, directionid):
        ...
        self.use_alt_day_string = False #When identical weekday/weekend trip and blocks are consolidated, use this
        self.alt_day_string = '' #When identical weekday/weekend trip and blocks are consolidated, use this
...
class Block:
   ...
   def pretty_print(self):
       if(trip_is_before_midday(self.triplist[0])):
          print(days_of_week_dict[self.serviceid] + ' AM Block (id: ' + self.blockid + ')')
       else:
          print(days_of_week_dict[self.serviceid] + ' PM Block (id: ' + self.blockid + ')')
       print('Routes: ' + str(self.get_block_routes()))
       print('================================')
       for trip in self.triplist:
          print(trip.starttime + ' | ' + trip.headsign + ' (Trip ID: ' + trip.tripid + ')')
       print('================================')
...
def populate_calendar():
    ...
    with open(calendarpath, 'r') as cal_f:
        ...
        for line in cal_f:
            ...
            days_of_week_dict_longname[service_id] = 'Special (id: ' + service_id + ')'
...
def start():
   ...
   with open(stoppath, 'r') as stops_f:
      ...
      colnames = stops_f.readline().split(',') #list of the column names, column order is in the first line
      for line in stops_f:
         items = line.rstrip().split(',')
   ...
   with open(routespath, 'r') as routes_f:
      #in the GTFS standard, the column order can CHANGE!!!!
      colnames = routes_f.readline().split(',') #list of the column names, column order is in the first line
   ...
   with open(stoptimespath, 'r') as stoptimes:
      ...
      for line in stoptimes:
         ...
         st = StopTime(tripid=tripid, stopid=stopid, stopname=stopname, departtime=depart_time, stopseq=stopseq)
   ...
   with open(tripspath, 'r') as trips:
      ...
      for line in trips:
         ...
         trip_obj = Trip(routeid=routeid, tripid=tripid, blockid=blockid, routenum=routenum, headsign=headsign, starttime=depart_time, startstopname=first_stop_name, serviceid=serviceid, directionid=directionid)
   ...
   print('Number of trips read: {0} number of valid trips: {1}'.format(total_trip_count, trip_pop_count))
   ...
   for key in blocklistdict:
      ...
      for tripid in blocklistdict[key]: #for each trip in this block
         new_block.triplist.append(tripdict[tripid]) #add that trip tuple to the block object
         #now, sort that trip list
         new_block.triplist.sort(key=trip_to_numseconds)
      new_block.serviceid = new_block.triplist[0].serviceid #take serviceid of first trip
   ...
   for st in stoptime_list:
      try:
         ...
         pass
   for trip in tripdict.values():
      trip.stoplist.sort(key=lambda x: hms_to_sec(x.departtime)) #sort the stoplist by stopseq didnt work so use departtime?

   print('Beginning stop->trip population...')
   for st in stoptime_list:
      stop = stopdict[st.stopid] #maintain these two lists in parallel (I know, I know...)
      if(st.tripid not in tripdict): #check for trips that arent from this sheet - dont want to put those in
          continue
      stop.triptimes.append((st.tripid, st.departtime))
   for stop in stopdict.values():
      stop.triptimes.sort(key = lambda x: hms_to_sec(x[1])) # sort trip tuple list by time
