...
def update_bus_history(bus):
    ...
    bus_history = BlockHistory(date, system_id, feed_version, block_id, routes, start_time, end_time)
...
def update_bus_history_old(bus):
    position = bus.position
    if bus.number is None or not position.active or position.trip is None:
        return
    history = load_bus_history(bus.number)
...
def load_bus_history(number):
    history_data = database.select('bus_history', ['date', 'system_id', 'feed_version', 'block_id', 'routes', 'start_time', 'end_time'], 'bus_number = ?', args=[number])
    history = []
