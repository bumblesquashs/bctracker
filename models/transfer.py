
from dataclasses import dataclass

from models.bus import Bus
from models.context import Context
from models.date import Date
from models.row import Row

@dataclass(slots=True)
class Transfer:
    '''Information about a bus moving from one system to another system'''
    
    id: int
    date: Date
    old_allocation_id: int
    old_context: Context
    old_bus: Bus
    new_allocation_id: int
    new_context: Context
    new_bus: Bus
    
    @classmethod
    def from_db(cls, row: Row):
        '''Returns a transfer initialized from the given database row'''
        id = row['id']
        old_allocation_id = row['old_allocation_id']
        old_context = row.context('old_allocation_agency_id', 'old_allocation_system_id')
        old_bus = old_context.find_bus(row['old_allocation_vehicle_id'])
        new_allocation_id = row['new_allocation_id']
        new_context = row.context('new_allocation_agency_id', 'new_allocation_system_id')
        new_bus = new_context.find_bus(row['new_allocation_vehicle_id'])
        date = Date.parse(row['date'], new_context.timezone)
        return cls(id, date, old_allocation_id, old_context, old_bus, new_allocation_id, new_context, new_bus)
